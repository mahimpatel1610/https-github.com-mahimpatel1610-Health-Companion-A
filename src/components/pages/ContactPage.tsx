...
export const ContactPage: React.FC<ContactPageProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSentNotice(
      `Message successfully delivered to ${doctorName}! It is now visible in their clinical portal and account inbox.`
    );
    setMessageText('');
  ...
  const userMessages = messages.filter((m) => {
    ...
    // Also show if sender name matches or seed message for demo
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              Your Clinic & Doctor Messages
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Track communication status and read verified doctor responses.
            </p>
          </div>
